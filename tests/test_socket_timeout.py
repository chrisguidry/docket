...
@skip_memory
@skip_cluster
async def test_connection_pool_disables_socket_read_timeout(docket: Docket):
...
@skip_memory
@skip_cluster
async def test_blocking_read_outlasts_redis_py_default_socket_timeout(docket: Docket):
    ...
    stream = docket.key("absent-stream")
    async with docket._redis.client() as r:
        start = time.monotonic()
        result = await r.xread({stream: "$"}, block=6_000)
