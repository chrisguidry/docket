...
instead, the way a deploy or a crash would, and start a second worker with no
...
async def stop_at_first_outage(
    worker: Worker, caplog: pytest.LogCaptureFixture
) -> None:
    """Run ``worker`` until it has hit Redis trouble once and started its
    reconnect wait, then stop it before it can recover.  The message it was
    handling stays pending in Redis for another worker."""
    run = asyncio.create_task(worker.run_forever())
    await wait_until(
        lambda: any(
            "Redis is unavailable" in record.getMessage() for record in caplog.records
        ),
        description="the worker to wait out its first Redis failure",
    )
    run.cancel()
...
async def test_fresh_worker_recovers_after_on_complete_failure(
    ...
    async with first_worker(docket) as worker_a:
        with patch.object(Perpetual, "on_complete", crashing_on_complete):
            await stop_at_first_outage(worker_a, caplog)
